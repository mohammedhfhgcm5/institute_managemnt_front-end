    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
          <div>
            <p className="text-muted-foreground">{text('تاريخ الميلاد', 'Date of Birth')}</p>
            <p className="font-medium">{formatDate(student.dateOfBirth)}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{text('الجنس', 'Gender')}</p>
            <p className="font-medium">{getStatusLabel(student.gender)}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{text('السنة الدراسية', 'Academic Year')}</p>
            <p className="font-medium">{student.academicYear || '-'}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{text('تاريخ التسجيل', 'Registration Date')}</p>
            <p className="font-medium">{formatDate(student.registrationDate)}</p>
          </div>
          {student.address && (
            <div className="col-span-2">
              <p className="text-muted-foreground">{text('العنوان', 'Address')}</p>
