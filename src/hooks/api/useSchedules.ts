...
export const useCreateSchedule = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(localize('تم إضافة الحصة بنجاح', 'Schedule created successfully'));
    },
...
export const useUpdateSchedule = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(localize('تم تحديث الحصة بنجاح', 'Schedule updated successfully'));
    },
    onError: (error: { response?: { data?: { message?: string } } }) => {
      toast.error(error.response?.data?.message || localize('حدث خطأ', 'Something went wrong'));
    },
...
export const useDeleteSchedule = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: scheduleKeys.all });
      toast.success(localize('تم حذف الحصة بنجاح', 'Schedule deleted successfully'));
    },
    onError: (error: { response?: { data?: { message?: string } } }) => {
      toast.error(error.response?.data?.message || localize('حدث خطأ', 'Something went wrong'));
    },
