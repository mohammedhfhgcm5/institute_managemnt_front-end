export const arTranslations = {
  locale: {
    english: "الإنجليزية",
    arabic: "العربية",
    switchToEnglish: "التبديل إلى الإنجليزية",
    switchToArabic: "التبديل إلى العربية",
  },
  "${format.toUpperCase()} downloaded successfully":
    "تم تنزيل ملف ${format.toUpperCase()} بنجاح",
  "${result.successCount} assessments created, ${result.failureCount} failed":
    "تم إنشاء ${result.successCount} تقييم، وفشل ${result.failureCount}",
  A: "أ",
  absences: "غيابات",
  Absent: "غائب",
  "Academic Year": "السنة الدراسية",
  "Academic Year 2024-2025 • Spring Semester":
    "السنة الدراسية 2024-2025 • الفصل الربيعي",
  "Account information": "معلومات الحساب",
  Actions: "الإجراءات",
  Active: "نشط",
  Add: "إضافة",
  "Add Assessment": "إضافة تقييم",
  "Add Class": "إضافة صف",
  "Add Course": "إضافة مادة",
  "Add Expense": "إضافة مصروف",
  "Add Link": "إضافة الربط",
  "Add New Teacher": "إضافة معلم جديد",
  "Add Parent": "إضافة ولي أمر",
  "Add Payment": "إضافة دفعة",
  "Add Schedule": "إضافة حصة",
  "Add Section": "إضافة شعبة",
  "Add Student": "إضافة طالب",
  "Add Subject": "إضافة مادة",
  "Add Teacher": "إضافة معلم",
  Address: "العنوان",
  Alert: "تنبيه",
  "All assessments were created successfully": "تم إنشاء جميع التقييمات بنجاح",
  "All notifications marked as read": "تم تعليم جميع الإشعارات كمقروءة",
  Amount: "المبلغ",
  "An unexpected error occurred": "حدث خطأ غير متوقع",
  Appearance: "المظهر",
  "Applied if student feedback is empty": "تُطبّق إذا كانت ملاحظة الطالب فارغة",
  Arabic: "العربية",
  "Are you sure you want to delete this assessment?":
    "هل أنت متأكد من حذف هذا التقييم؟",
  "Are you sure you want to delete this attendance record?":
    "هل أنت متأكد من حذف سجل الحضور هذا؟",
  "Are you sure you want to delete this class?":
    "هل أنت متأكد من حذف هذا الصف؟",
  "Are you sure you want to delete this expense?":
    "هل أنت متأكد من حذف هذا المصروف؟",
  "Are you sure you want to delete this parent?":
    "هل أنت متأكد من حذف ولي الأمر هذا؟",
  "Are you sure you want to delete this payment?":
    "هل أنت متأكد من حذف هذه الدفعة؟",
  "Are you sure you want to delete this report?":
    "هل أنت متأكد من حذف هذا التقرير؟",
  "Are you sure you want to delete this schedule?":
    "هل أنت متأكد من حذف هذه الحصة؟",
  "Are you sure you want to delete this section?":
    "هل أنت متأكد من حذف هذه الشعبة؟",
  "Are you sure you want to delete this student? This action cannot be undone.":
    "هل أنت متأكد من حذف هذا الطالب؟ لا يمكن التراجع عن هذا الإجراء.",
  "Are you sure you want to delete this subject-grade relation?":
    "هل أنت متأكد من حذف هذا الربط؟",
  "Are you sure you want to delete this subject?":
    "هل أنت متأكد من حذف هذه المادة؟",
  "Are you sure you want to delete this teacher?":
    "هل أنت متأكد من حذف هذا المعلم؟",
  "Assessment created successfully": "تم إضافة التقييم بنجاح",
  "Assessment deleted successfully": "تم حذف التقييم بنجاح",
  "Assessment Management": "إدارة التقييمات",
  "Assessment updated successfully": "تم تحديث التقييم بنجاح",
  Attendance: "الحضور",
  "Attendance Calendar": "تقويم الحضور",
  "Attendance calendar will be added soon": "سيتم إضافة تقويم الحضور قريباً",
  "Attendance Management": "إدارة الحضور",
  "Attendance Rate": "نسبة الحضور",
  "Attendance record deleted successfully": "تم حذف سجل الحضور بنجاح",
  "Attendance Report": "تقرير حضور",
  "Attendance saved successfully": "تم تسجيل الحضور بنجاح",
  "Attendance Summary": "ملخص الحضور",
  "Attendance updated successfully": "تم تحديث الحضور بنجاح",
  "Available Modules": "الحصص المتاحة",
  Average: "المعدل",
  "Back to Schedules": "العودة للجداول",
  Bio: "السيرة الذاتية",
  "Builder View": "عرض المُنشئ",
  "Bulk Assessment Scores": "إدخال درجات التقييم دفعة واحدة",
  "Bulk Attendance": "تسجيل حضور جماعي",
  "Bulk attendance has been saved": "تم حفظ الحضور الجماعي",
  Cancel: "إلغاء",
  Cancelled: "ملغاة",
  Capacity: "السعة",
  Category: "التصنيف",
  "Change password": "تغيير كلمة المرور",
  "Change Section": "تغيير الشعبة",
  Channel: "القناة",
  "Choose grade first": "اختر الصف أولاً",
  "Choose grade...": "اختر الصف...",
  "Choose section first": "اختر الشعبة أولاً",
  "Choose section...": "اختر الشعبة...",
  "Class Management": "إدارة الصفوف",
  "Class Name": "اسم الصف",
  Close: "إغلاق",
  Completed: "مكتملة",
  Confirm: "تأكيد",
  "Confirm password": "تأكيد كلمة المرور",
  "Course Name": "اسم المادة",
  Create: "إنشاء",
  "Create New Report": "إنشاء تقرير جديد",
  "Create Report": "إنشاء تقرير",
  "Created At": "تاريخ الإنشاء",
  "Created at:": "تاريخ الإنشاء:",
  "Current mode": "الوضع الحالي",
  "Current password": "كلمة المرور الحالية",
  "Current user": "المستخدم الحالي",
  Dark: "داكن",
  "Dark mode": "الوضع الداكن",
  Dashboard: "لوحة التحكم",
  Data: "البيانات",
  Date: "التاريخ",
  "Date of Birth": "تاريخ الميلاد",
  Day: "اليوم",
  "Default Feedback (optional)": "ملاحظات افتراضية (اختياري)",
  Delete: "حذف",
  "Delete Assessment": "حذف التقييم",
  "Delete attendance": "حذف الحضور",
  "Delete Attendance Record": "حذف سجل الحضور",
  "Delete Class": "حذف الصف",
  "Delete Expense": "حذف المصروف",
  "Delete Grade-Subject Link": "حذف ربط الصف والمادة",
  "Delete Parent": "حذف ولي الأمر",
  "Delete Payment": "حذف الدفعة",
  "Delete Report": "حذف التقرير",
  "Delete Schedule": "حذف الحصة",
  "Delete Section": "حذف الشعبة",
  "Delete Student": "حذف الطالب",
  "Delete Subject": "حذف المادة",
  "Delete Teacher": "حذف المعلم",
  Description: "الوصف",
  Discount: "الخصم",
  "Download report": "تنزيل التقرير",
  "Drag modules from the library and drop them onto the schedule grid to assign classes.":
    "اسحب الحصص من المكتبة وأفلتها داخل الجدول لتوزيع الحصص الدراسية.",
  "DRAG TO GRID": "اسحب إلى الجدول",
  "Drop here": "أسقط هنا",
  "Due Date": "تاريخ الاستحقاق",
  "Each row links one subject to one grade and one section, with required teacher.":
    "كل سطر يربط مادة بصف وشعبة ومعلم.",
  "Edit Assessment": "تعديل التقييم",
  "Edit attendance": "تعديل الحضور",
  "Edit Attendance": "تعديل الحضور",
  "Edit Course": "تعديل المادة",
  "Edit Expense": "تعديل المصروف",
  "Edit Parent": "تعديل ولي الأمر",
  "Edit Payment": "تعديل الدفعة",
  "Edit Schedule": "تعديل الحصة",
  "Edit Section": "تعديل الشعبة",
  "Edit Student": "تعديل الطالب",
  "Edit Teacher": "تعديل المعلم",
  Email: "بريد إلكتروني",
  "Email:": "البريد الإلكتروني:",
  "End Time": "وقت النهاية",
  English: "الإنجليزية",
  "Enter title": "أدخل العنوان",
  Environment: "البيئة",
  Exam: "اختبار",
  "Example: 12": "مثال: 12",
  Excused: "معذور",
  "Expense created successfully": "تم إضافة المصروف بنجاح",
  "Expense deleted successfully": "تم حذف المصروف بنجاح",
  "Expense Management": "إدارة المصروفات",
  "Expense Statistics": "إحصائيات المصروفات",
  "Expense updated successfully": "تم تحديث المصروف بنجاح",
  Expenses: "المصروفات",
  "Expenses by Category": "المصروفات حسب التصنيف",
  "Experience Years": "سنوات الخبرة",
  "Export Excel": "تصدير Excel",
  "Export PDF": "تصدير PDF",
  "Failed to change password": "فشل تغيير كلمة المرور",
  "Failed to create assessments": "فشل إنشاء التقييمات",
  "Failed to create student": "حدث خطأ أثناء إضافة الطالب",
  "Failed to delete student": "حدث خطأ أثناء الحذف",
  "Failed to export report file.": "فشل تصدير ملف التقرير.",
  "Failed to update student": "حدث خطأ أثناء التحديث",
  Father: "أب",
  Feedback: "الملاحظات",
  Female: "أنثى",
  "Fill All = 0": "تعبئة الكل = 0",
  "Fill All = Max Score": "تعبئة الكل = الدرجة العظمى",
  Final: "اختبار نهائي",
  "Financial Report": "تقرير مالي",
  "First name": "الاسم الأول",
  "First Name": "الاسم الأول",
  "First Term Exam": "اختبار الفصل الأول",
  Format: "الصيغة",
  From: "من",
  "From date": "من تاريخ",
  Gender: "الجنس",
  "Generated At": "تاريخ التوليد",
  Grade: "الصف",
  "Grade Statistics": "إحصائيات الدرجات",
  "Grade Subject": "مادة الصف",
  "Grade-subject link created successfully": "تم إنشاء ربط مادة الصف بنجاح",
  "Grade-subject link deleted successfully": "تم حذف ربط مادة الصف بنجاح",
  "Grade-subject link updated successfully": "تم تحديث ربط مادة الصف بنجاح",
  "Grade-Subject Relation (grade_subject)":
    "ربط الصفوف بالمواد (grade_subject)",
  Graduated: "متخرج",
  Guardian: "ولي أمر",
  "Highest Score": "أعلى درجة",
  "Hire Date": "تاريخ التوظيف",
  Homework: "واجب",
  "In app": "داخل التطبيق",
  Inactive: "غير نشط",
  Income: "الدخل",
  Info: "معلومات",
  Language: "اللغة",
  "Language & Region": "اللغة والمنطقة",
  "Last name": "اسم العائلة",
  "Last Name": "اسم العائلة",
  Late: "متأخر",
  "Late Minutes": "دقائق التأخير",
  "Left to right (LTR)": "من اليسار إلى اليمين (LTR)",
  Level: "المستوى",
  Light: "فاتح",
  "Loading grade subjects...": "جاري تحميل مواد الصف...",
  "Loading grades...": "جاري تحميل الصفوف...",
  "Loading schedules...": "جاري تحميل الحصص...",
  "Loading sections...": "جاري تحميل الشعب...",
  "Loading students...": "جاري تحميل الطلاب...",
  "Loading...": "جاري التحميل...",
  "Logged in successfully": "تم تسجيل الدخول بنجاح",
  Login: "تسجيل الدخول",
  "Login failed": "فشل تسجيل الدخول",
  Logout: "تسجيل الخروج",
  "Lowest Score": "أقل درجة",
  Maintenance: "صيانة",
  Male: "ذكر",
  "Mark All Absent": "تحديد الكل غائب",
  "Mark all as read": "تعليم الكل كمقروء",
  "Mark All Excused": "تحديد الكل معذور",
  "Mark All Late": "تحديد الكل متأخر",
  "Mark All Present": "تحديد الكل حاضر",
  "Max Score": "الدرجة العظمى",
  "Max score must be greater than zero":
    "يجب أن تكون الدرجة العظمى أكبر من صفر",
  "Max Students": "الحد الأقصى للطلاب",
  Message: "الرسالة",
  Midterm: "اختبار نصفي",
  "Module Library": "مكتبة الحصص",
  "Monthly Financial Summary": "الملخص المالي الشهري",
  Mother: "أم",
  "My Account": "حسابي",
  "My Notifications": "إشعاراتي",
  Name: "الاسم",
  Net: "الصافي",
  "Net Balance": "صافي الرصيد",
  new: "جديد",
  "New password": "كلمة المرور الجديدة",
  Next: "التالي",
  "No absentee records found.": "لا توجد سجلات غياب.",
  "No data available": "لا توجد بيانات",
  "No downloadable file is available for this report.":
    "لا يوجد ملف قابل للتنزيل لهذا التقرير.",
  "No expense categories found for this month.":
    "لا توجد مصروفات مسجلة لهذا الشهر.",
  "No JSON data available to export.": "لا توجد بيانات JSON متاحة للتصدير.",
  "No modules found for this section": "لا توجد حصص متاحة لهذه الشعبة",
  "No notifications": "لا توجد إشعارات",
  "No recent payments found.": "لا توجد مدفوعات حديثة.",
  "No room": "بدون غرفة",
  "No sections for selected grade": "لا توجد شعب للصف المحدد",
  "No students found for selected section": "لا يوجد طلاب في الشعبة المحددة",
  "No students found for the selected section": "لا يوجد طلاب للشعبة المحددة",
  "No students found for this section": "لا يوجد طلاب لهذه الشعبة",
  "No teacher": "بدون معلم",
  "Not set": "غير محدد",
  Notes: "ملاحظات",
  "Notification content": "محتوى الإشعار",
  "Notification sent successfully": "تم إرسال الإشعار بنجاح",
  "Notification title": "عنوان الإشعار",
  Notifications: "الإشعارات",
  "Notifications sent successfully": "تم إرسال الإشعارات بنجاح",
  of: "من",
  Optional: "اختياري",
  "Optional note": "ملاحظة اختيارية",
  Other: "أخرى",
  Page: "صفحة",
  Paid: "مدفوع",
  Parameters: "المعلومات",
  Parent: "ولي الأمر",
  "Parent created successfully": "تم إضافة ولي الأمر بنجاح",
  "Parent deleted successfully": "تم حذف ولي الأمر بنجاح",
  "Parent Management": "إدارة أولياء الأمور",
  "Parent updated successfully": "تم تحديث بيانات ولي الأمر بنجاح",
  Parents: "أولياء الأمور",
  Partial: "جزئي",
  "Pass Rate": "نسبة النجاح",
  Password: "كلمة المرور",
  "Password changed successfully": "تم تغيير كلمة المرور بنجاح",
  "Payment created successfully": "تم إضافة الدفعة بنجاح",
  "Payment Date": "تاريخ الدفع",
  "Payment deleted successfully": "تم حذف الدفعة بنجاح",
  "Payment updated successfully": "تم تحديث الدفعة بنجاح",
  Payments: "المدفوعات",
  Pending: "معلق",
  Percentage: "النسبة",
  "Performance Report": "تقرير أداء",
  "Period End": "نهاية الفترة",
  "Period Start": "بداية الفترة",
  Phone: "الهاتف",
  "Phone:": "رقم الهاتف:",
  "Please choose a schedule": "يرجى اختيار الحصة أولاً",
  "Please choose a section first": "يرجى اختيار الشعبة أولاً",
  "Please choose attendance date": "يرجى اختيار تاريخ الحضور",
  "Please choose grade first": "يرجى اختيار الصف أولاً",
  "Please choose grade subject": "يرجى اختيار مادة الصف",
  "Please enter a valid user ID": "يرجى إدخال رقم مستخدم صالح",
  "Please enter assessment title": "يرجى إدخال عنوان التقييم",
  "Please enter score for all students (${missingScores.length} missing)":
    "يرجى إدخال الدرجة لجميع الطلاب (المتبقي ${missingScores.length})",
  "Please select grade, section, subject, and teacher.":
    "يرجى اختيار الصف والشعبة والمادة والمعلم.",
  Preparatory: "إعدادي",
  present: "حاضر",
  Present: "حاضر",
  Previous: "السابق",
  Production: "إنتاج",
  Profile: "الملف الشخصي",
  Qualifications: "المؤهلات",
  "Quick Actions": "إجراءات سريعة",
  "Quick Tip": "نصيحة سريعة",
  Quiz: "اختبار قصير",
  Receipt: "رقم الإيصال",
  "Receipt No.": "رقم الإيصال",
  "Recent Payments": "أحدث المدفوعات",
  "Record Attendance": "تسجيل حضور",
  Refresh: "تحديث",
  "Registration Date": "تاريخ التسجيل",
  Relationship: "نوع العلاقة",
  "Report created successfully": "تم إنشاء التقرير بنجاح",
  "Report deleted successfully": "تم حذف التقرير بنجاح",
  "Report Details": "تفاصيل التقرير",
  "Report Generator": "مولد التقارير",
  "Report ID": "رقم التقرير",
  "Report Title": "عنوان التقرير",
  "Report Type": "نوع التقرير",
  Reports: "التقارير",
  Retry: "إعادة المحاولة",
  "Right to left (RTL)": "من اليمين إلى اليسار (RTL)",
  Role: "الصلاحية",
  "Role:": "الصلاحية:",
  Room: "الغرفة",
  "Room 101": "الغرفة 101",
  Salaries: "رواتب",
  Salary: "الراتب",
  Save: "تسجيل",
  Schedule: "الحصة",
  "Schedule Builder": "منشئ الجداول",
  "Schedule created successfully": "تم إضافة الحصة بنجاح",
  "Schedule deleted successfully": "تم حذف الحصة بنجاح",
  "Schedule ID": "رقم الجدول",
  "Schedule Management": "إدارة الجداول",
  "Schedule updated successfully": "تم تحديث الحصة بنجاح",
  Scheduled: "مجدولة",
  "School Admin": "إدارة المدرسة",
  "School Management System": "نظام إدارة المدرسة",
  "School Management System. All rights reserved.":
    "نظام إدارة المدرسة. جميع الحقوق محفوظة.",
  Score: "الدرجة",
  "Search ${label}...": "ابحث عن ${label}...",
  "Search assessments...": "ابحث عن تقييم...",
  "Search attendance...": "بحث في الحضور...",
  "Search class...": "ابحث عن صف...",
  "Search modules...": "ابحث عن الحصص...",
  "Search parent...": "بحث عن ولي أمر...",
  "Search schedules...": "ابحث عن جدول...",
  "Search sections...": "ابحث عن شعبة...",
  "Search students...": "ابحث عن الطلاب...",
  "Search subject...": "ابحث عن مادة...",
  "Search teachers...": "ابحث عن المعلمين...",
  "Search...": "بحث...",
  Secondary: "ثانوي",
  Section: "الشعبة",
  "Section created successfully": "تم إضافة الشعبة بنجاح",
  "Section deleted successfully": "تم حذف الشعبة بنجاح",
  "Section Management": "إدارة الشعب",
  "Section Name": "اسم الشعبة",
  "Section updated successfully": "تم تحديث الشعبة بنجاح",
  Sections: "الشعب",
  "Select channel": "اختر القناة",
  "Select grade": "اختر الصف",
  "Select grade first": "اختر الصف أولاً",
  "Select grade first, then choose a section to build its schedule.":
    "اختر الصف أولاً ثم اختر الشعبة لبناء جدولها.",
  "Select grade subject": "اختر مادة الصف",
  "Select schedule": "اختر الحصة",
  "Select section": "اختر الشعبة",
  "Select section and schedule, then submit attendance for all students in one request.":
    "اختر الشعبة والحصة ثم أرسل حضور جميع الطلاب في طلب واحد.",
  "Select section, choose grade subject, then enter each student score and submit once.":
    "اختر الشعبة ثم مادة الصف ثم أدخل درجات الطلاب وأرسل مرة واحدة.",
  "Select student": "اختر الطالب",
  "Select type": "اختر النوع",
  Selected: "المحدد",
  Send: "إرسال",
  "Send Notification": "إرسال إشعار",
  Settings: "الإعدادات",
  Showing: "عرض",
  "Sign in to access the dashboard": "قم بتسجيل الدخول للوصول إلى لوحة التحكم",
  SMS: "رسالة نصية",
  "Something went wrong": "حدث خطأ",
  Specialization: "التخصص",
  "Start Time": "وقت البداية",
  Status: "الحالة",
  "Status:": "الحالة:",
  Student: "الطالب",
  "Student created successfully": "تم إضافة الطالب بنجاح",
  "Student deleted successfully": "تم حذف الطالب بنجاح",
  "Student ID": "رقم الطالب",
  "Student Management": "إدارة الطلاب",
  "Student updated successfully": "تم تحديث بيانات الطالب بنجاح",
  Students: "الطلاب",
  Subject: "المادة",
  "Subject created successfully": "تم إضافة المادة بنجاح",
  "Subject deleted successfully": "تم حذف المادة بنجاح",
  "Subject Management": "إدارة المواد",
  "Subject Name": "اسم المادة",
  "Subject updated successfully": "تم تحديث المادة بنجاح",
  "Submit Bulk Attendance": "إرسال الحضور دفعة واحدة",
  "Submit Bulk Scores": "إرسال الدرجات دفعة واحدة",
  Success: "نجاح",
  Supplies: "مستلزمات",
  "System information": "معلومات النظام",
  "Take Attendance": "تسجيل حضور",
  Teacher: "المعلم",
  "Teacher created successfully": "تم إضافة المعلم بنجاح",
  "Teacher deleted successfully": "تم حذف المعلم بنجاح",
  "Teacher Details": "تفاصيل المعلم",
  "Teacher Management": "إدارة المعلمين",
  "Teacher Schedule": "جدول المعلم",
  "Teacher schedule will be added soon": "سيتم إضافة جدول المعلم قريباً",
  "Teacher updated successfully": "تم تحديث بيانات المعلم بنجاح",
  "Text direction": "اتجاه النص",
  "This subject is already linked to the selected grade.":
    "هذه المادة مرتبطة بالفعل بالصف المختار.",
  Time: "الوقت",
  TIME: "الوقت",
  Title: "العنوان",
  To: "إلى",
  "To date": "إلى تاريخ",
  "Today Attendance": "حضور اليوم",
  "Toggle between light and dark mode": "تبديل بين الوضع الفاتح والداكن",
  "Top Absentees": "الأكثر غياباً",
  "Total Expenses": "إجمالي المصروفات",
  "Total Paid": "إجمالي المدفوعات",
  "Total Records": "إجمالي السجلات",
  "Total Students": "إجمالي الطلاب",
  "Total Teachers": "إجمالي المعلمين",
  Type: "النوع",
  Unknown: "غير معروف",
  "Unread Notifications": "الإشعارات غير المقروءة",
  Update: "تحديث",
  "Updated At": "آخر تحديث",
  "Use the reports page to create a new report":
    "استخدم صفحة التقارير لإنشاء تقرير جديد",
  "User created successfully": "تم إنشاء المستخدم بنجاح",
  "User deleted successfully": "تم حذف المستخدم بنجاح",
  "User ID": "رقم المستخدم",
  "User updated successfully": "تم تحديث المستخدم بنجاح",
  Utilities: "مرافق",
  Version: "الإصدار",
  "View all notifications": "عرض جميع الإشعارات",
  Warning: "تحذير",
  StudentPayments: "دفعات الطالب",
  RecentAssessments: "آخر التقييمات",
  RecentAttendance: "آخر الحضور",
  Remaining: "المتبقي",
  AnnualTuition: "القسط السنوي",
  Account: "الحساب",
  "Account already linked": "الحساب مرتبط بالفعل",
  "Add Reception": "إضافة استقبال",
  "Create Account": "إنشاء حساب",
  "Create Reception Account": "إنشاء حساب استقبال",
  "Delete Reception": "حذف الاستقبال",
  "Edit Reception": "تعديل الاستقبال",
  "Email must match the reception profile.": "يجب أن يطابق البريد الإلكتروني ملف الاستقبال.",
  Linked: "مرتبط",
  "Not linked": "غير مرتبط",
  "Reception Management": "إدارة الاستقبال",
  "Reception account created successfully": "تم إنشاء حساب الاستقبال بنجاح",
  "Reception created successfully": "تم إضافة الاستقبال بنجاح",
  "Reception deleted successfully": "تم حذف الاستقبال بنجاح",
  "Reception updated successfully": "تم تحديث بيانات الاستقبال بنجاح",
  "Search reception...": "ابحث عن موظف الاستقبال..."
};